...
var APP = {
	...
	Player: function () {
		...
		this.load = function (json) {
			...
			scene.getObjectByName("fart").name = "fart0"
			for (let ix = 0; ix < OBJEKTUMLISTA.length - 1; ix++) {
				...
				asd.children[1].children[0].name = ("fart" + (ix+1))
				scene.add(asd);
				// console.log(asd.children[1].children[0].name);
				// console.log();
			}
			const raycaster = new THREE.Raycaster();
			const clickMouse = new THREE.Vector2();
			window.addEventListener("click", event => {
				clickMouse.x = (event.clientX / $("#macska").width()) * 2 - 1;
				clickMouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
				raycaster.setFromCamera(clickMouse, camera);
				const found = raycaster.intersectObjects(scene.children);
				// console.log(scene.children);
				for (let ix = 0; ix < OBJEKTUMLISTA.length; ix++) {
					if (found.length > 0 && found[0].object.name == ("fart"+ix)) {
						var selectedObject = scene.getObjectByName("fart"+ix);
						var masik = selectedObject.parent.clone();
						//masik.scale.set(300,300,600);
						var asd = scene.getObjectByName("fart"+ix);
						// console.log(scene.getObjectByName("fart"+ix));
						//const ido = new THREE.Clock();
						ido.start();
						// console.log(scene.getObjectByName("fart"+ix));
						asd.parent.parent.add(masik);
